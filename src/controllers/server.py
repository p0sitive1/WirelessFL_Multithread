...
import os
from src.trainers.fedavg13 import FedAvg13Trainer
from src.trainers.fedavg12 import FedAvg12Trainer
from src.trainers.fedavg9 import FedAvg9Trainer
from src.trainers.fedavg4 import FedAvg4Trainer
...
from src.communication.comm_tcp_multithread import TCP_SOCKET_MULTI
...
from src.trainers.fedavg6 import FedAvg6Trainer
from src.trainers.basicTrainer import FedAvgTrainer
...
class FedIoTServer:
    ...
    def config_experiment(self, args):
        ...
        if dataset == 'synthetic':
            ...
            if self.model == 'logistic':
                self.args.update({'input_shape': 3072, 'num_class': 10})
            else:
                self.args.update({'input_shape':(3,32,32), 'num_class':10})


        # Trainer
        model = choose_model(self.args)
        # self.optimizer = GD(model.parameters(), lr=self.args['lr'], weight_decay=self.args['wd'])
        self.optimizer = torch.optim.Adam(model.parameters(), lr=self.args['lr'], weight_decay=self.args['wd'])
        self.num_epoch = self.args['num_epoch']
        if self.algo != 'uniform':
    ...
    def connect2clients(self, ):
        ...
        self.socketServer = TCP_SOCKET_MULTI(self.IP, self.PORT)
    ...
    def test_comm_speed(self, ):
        ...
        with open('./log/time.json','w') as fp:
    ...
    def train(self, ):
        ...
        if self.algo == 'basic':
            print("IS basic")
            self.trainer = FedAvgTrainer(self.args, trainerConfig)
        elif self.algo == 'proposed':
            self.trainer = FedAvg6Trainer(self.args, trainerConfig)
        elif self.algo == 'statistical':
            self.trainer = FedAvg13Trainer(self.args, trainerConfig)
        elif self.algo == 'weighted':
            self.trainer = FedAvg4Trainer(self.args, trainerConfig)
        elif self.algo == 'uniform':
            self.trainer = FedAvg9Trainer(self.args, trainerConfig)
        elif self.algo == 'full':
            self.trainer = FedAvg12Trainer(self.args, trainerConfig)
        else:
